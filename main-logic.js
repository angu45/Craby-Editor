/* CRABY EDITOR - FULL LOGIC 
   Features: Persistent Settings, Multi-file Management, 
   Refreshed Download Logic & Theme Sync.
*/
...
window.resetAllSettings = () => {
    if(confirm("सगळ्या सेटिंग्ज रिसेट करायच्या आहेत का?")) {
        localStorage.clear();
        location.reload();
    }
};
...
window.deleteFile = (id) => {
    if(confirm(`${id} फाईल डिलीट करायची?`)) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const defC = localStorage.getItem('craby_code_css') || "h1 { color: #ffb400; text-align: center; }";
    const defJ = localStorage.getItem('craby_code_js') || "console.log('Live!');";
